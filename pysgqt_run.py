...
def build_layout(data, info):
    ...
    layout.extend([[sg.Text('Output directory:', font=fstd_bold), sg.In(size=(25,1), enable_events=True, default_text=cwd, key='output-dir'),sg.FolderBrowse(initial_folder=cwd)],
    ...
    for k in data:
        ...
        # otherwise use
        row = [sg.Text(k)] # push to align right
        if e['gtype'] == 'SCALE':
